...
const Investments = () => {
  ...
  const getData = () => {
    let url =
      "https://api.sheety.co/33d9ec27f5c7dfb130eb655baacab48d/november2022Api/investments";
    fetch(url)
      .then((response) => response.json())
      .then((json) => {
        // Do something with the data
        const data = json.investments;
        setInvestmentsData(data.slice(1, -2));
        setTotal(data.slice(-2)[0]);
        console.log(data, "data");
        console.log(data.slice(1, -2), "setinvestmentsData");
